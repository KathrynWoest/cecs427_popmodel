...
5. **plot.py:** Plots a line graph of the number of new infections per day after the simulation has completed. Takes in the user's graph, a list of graphs representing the results after every round in the simulation, and the mode of the simulation (either `cascade` or `covid`). This information is used to calculate the number of new infections per day.
6. **interactive.py:** Visualizes the graph and the state of the nodes at every round in the simulation. Like `plot()`, `show_interactive()` takes in the user's graph, a list of graphs representing the results after every round in the simulation, and the mode of the simulation (either `cascade` or `covid`). Includes a slider to scrub through each round, showing the progression of the infection.
